 ...
 * 具体来说，小于 −231 的整数应该被舍入为 −231 ，大于 231 − 1 的整数应该被舍入为 231 − 1 。返回整数作为最终结果。
 * 
 * 示例 1：
 * 输入：s = "42"
 * 输出：42
...
public class Number0008 {
    public int myAtoi(String str) {
        ...
        return new Version1().invoke(str);
    }
    ...
    public static class Version2 {
        ...
        public static class Automaton {
            ...
            private static Map<String, List<String>> table = new HashMap<String, List<String>>() {{
            ...
            public boolean accept(char c) {
                state = table.get(state).get(figureIndex(c));
                if ("end".equals(state)) {
                    return false;
                }
                ...
                if ("number".equals(state)) {
                    value = value * 10 + c - '0';
                    value = (sign == 1) ? 
                        Math.min(value, (long) Integer.MAX_VALUE) : 
                        Math.min(value, -(long) Integer.MIN_VALUE);
            ...
            private int figureIndex(char c) {
                if (c == ' ') {
                    return 0;
                }
                if (c == '+' || c == '-') {
